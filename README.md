...
**온라인 대전을 쓰려면** 중계 서버가 필요합니다. 한 번만 준비해 두면 위 실행 방법 그대로 쓸 수 있습니다.

```bash
cd server && npm install
```

`tools/launch.py` 는 Node 와 `server/node_modules` 가 보이면 중계 서버를 띄우고(온라인 대전까지 가능), 없으면 파이썬 정적 서버로 떨어집니다(AI · 2인 대전만). 어느 쪽인지 실행할 때 콘솔에 찍힙니다.
...
> [!TIP]
> 🟩 **수정한 내용 — 음향 조절과 게임 나가기는 구현, 항복은 없습니다.**
>
...
| 난이도 | 시간 | 실제 도달 깊이 | 성격 |
|---|---|---|---|
| 초급 | 0.2초 | 2수 | 판단이 거칠고 14% 확률로 실수합니다 |
| 중급 | 0.9초 | 3~4수 | 정석을 따르고 실수는 거의 없습니다 |
| 고급 | 2.6초 | 5~6수 | 실수하지 않고, 증강도 판 상황에 맞춰 고릅니다 |
...
>
> 난이도별 잡음·실수 확률은 건드리지 않았습니다. 초급은 여전히 14% 확률로 일부러 틀립니다.
