...
## 중계 서버를 파이썬으로 옮김 (2026-09-10)

Node + ws 로 만들었더니 `node_modules` 가 저장소에 안 올라가서, **다른 컴퓨터에서 받으면
온라인이 조용히 꺼져 있었다.** 실행기가 정적 서버로 떨어지고 "이 주소에서는 온라인 대전을
쓸 수 없습니다" 만 떴다.

→ `server/relay.py` 한 파일로 다시 썼다. 파이썬 표준 라이브러리만 쓴다
   (WebSocket 핸드셰이크 = sha1+base64, 프레임 디코드/인코드, 스레드 per 연결).
   프로토콜은 그대로라 클라이언트(net.js)는 한 줄도 안 바뀌었다.
   `server/index.js` · `package.json` 은 지웠다 — 같은 프로토콜을 두 번 유지하면 어긋난다.

배포도 파이썬으로 하면 된다 (Render 는 파이썬 웹 서비스를 지원한다).
시작 명령: `python server/relay.py` (PORT 환경변수를 읽는다).


## 함정: 같은 포트에 서버가 둘 (2026-09-10)

증상은 "코드 입력했는데 방이 없대". 원인은 서버가 두 개 떠 있던 것.

**Windows 의 SO_REUSEADDR 는 리눅스와 다르다.** 이미 듣고 있는 포트에 또 바인드하는 것까지
허용한다. `allow_reuse_address = True` 를 그대로 켜 뒀더니 실행 버튼을 두 번 누른 것만으로
같은 포트에 서버가 둘 떴고, 접속이 둘로 갈렸다. 방은 프로세스 메모리에 있으니
방을 만든 사람과 들어오는 사람이 다른 프로세스에 붙으면 방을 찾을 수 없다.

세 겹으로 막았다.
1. `relay.py` — Windows 에서는 `SO_EXCLUSIVEADDRUSE`. 두 번째 바인드가 실패한다.
   실패하면 조용히 죽지 말고 왜인지 찍는다.
2. `launch.py` 의 빈 포트 탐색 — 같은 옵션을 줘야 '쓰고 있는 포트'를 비었다고 넘겨주지 않는다.
3. `launch.py` — 새로 띄우기 전에 `/healthz` 로 이미 떠 있는 무제체스를 찾아보고,
   있으면 그 주소를 열기만 한다. (`/healthz` 가 `muje-chess ok` 를 돌려주는 이유)
